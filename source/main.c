...
static void btn_task_sem(void *arg)
{
    for (;;) {
        if (xSemaphoreTake(btn_sem, portMAX_DELAY) == pdTRUE) {
             ...
             */
            ESP_LOGI(TAG, "[sem] Drop Tower E-STOP pressed —> assert safe state, press #%lu  latency=%lld us (max=%llu)",
...
static void btn_task_notif(void *arg)
{
    for (;;) {
        ...
        ESP_LOGI(TAG, "[notif] Drop Tower E-STOP pressed —> assert safe state, press #%lu  latency=%lld us (max=%llu) "
                      "(notif count=%lu)",
                 (unsigned long)presses_observed,
                 (long long)lat,
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "==== App 3 [Industrial: Drop Tower Controller] starting — ISR + bottom-half ====");
